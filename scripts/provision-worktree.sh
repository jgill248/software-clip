...
base_cwd="${SOFTCLIP_WORKSPACE_BASE_CWD:?SOFTCLIP_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${SOFTCLIP_WORKSPACE_CWD:?SOFTCLIP_WORKSPACE_CWD is required}"
softclip_home="${SOFTCLIP_HOME:-$HOME/.softclip}"
softclip_instance_id="${SOFTCLIP_INSTANCE_ID:-default}"
softclip_dir="$worktree_cwd/.softclip"
worktree_config_path="$softclip_dir/config.json"
worktree_env_path="$softclip_dir/.env"
worktree_name="${SOFTCLIP_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${SOFTCLIP_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.softclip/config.json" || -L "$base_cwd/.softclip/config.json" ) ]]; then
  source_config_path="$base_cwd/.softclip/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$softclip_home/instances/$softclip_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm softclip --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm softclip worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v softclip >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      softclip worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  SOFTCLIP_DIR="$softclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  SOFTCLIP_WORKTREES_DIR="${SOFTCLIP_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const softclipDir = process.env.SOFTCLIP_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.SOFTCLIP_WORKTREES_DIR) ?? "~/.softclip-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(softclipDir, "config.json");
  const envPath = path.resolve(softclipDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "softclip",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.SOFTCLIP_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.SOFTCLIP_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.SOFTCLIP_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "SOFTCLIP_HOME=" + JSON.stringify(worktreeHome),
    "SOFTCLIP_INSTANCE_ID=" + JSON.stringify(instanceId),
    "SOFTCLIP_CONFIG=" + JSON.stringify(configPath),
    "SOFTCLIP_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "SOFTCLIP_IN_WORKTREE=true",
    "SOFTCLIP_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.SOFTCLIP_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("SOFTCLIP_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if softclipai_command_available; then
  run_isolated_worktree_init
else
  echo "softclip CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".softclip-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
