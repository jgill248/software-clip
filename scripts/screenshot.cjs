#!/usr/bin/env node
/**
 * Screenshot utility for Softclip UI.
 *
 * Reads the board token from ~/.softclip/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/softclip-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".softclip/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.softclip/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Softclip origin only
