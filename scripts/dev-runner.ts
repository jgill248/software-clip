...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[softclip] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`softclip worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".softclip", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".softclip/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[softclip] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[softclip] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[softclip] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  SOFTCLIP_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.SOFTCLIP_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.SOFTCLIP_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.SOFTCLIP_MIGRATION_PROMPT ??= "never";
  env.SOFTCLIP_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[softclip] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.SOFTCLIP_BIND = effectiveBind;
  if (bindHost) {
    env.SOFTCLIP_BIND_HOST = bindHost;
  } else {
    delete env.SOFTCLIP_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.SOFTCLIP_DEPLOYMENT_MODE;
    delete env.SOFTCLIP_DEPLOYMENT_EXPOSURE;
    delete env.SOFTCLIP_AUTH_BASE_URL_MODE;
    console.log("[softclip] dev mode: local_trusted (bind=loopback)");
  } else {
    env.SOFTCLIP_DEPLOYMENT_MODE = "authenticated";
    env.SOFTCLIP_DEPLOYMENT_EXPOSURE = "private";
    env.SOFTCLIP_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[softclip] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.SOFTCLIP_BIND;
  delete env.SOFTCLIP_BIND_HOST;
  delete env.SOFTCLIP_DEPLOYMENT_MODE;
  delete env.SOFTCLIP_DEPLOYMENT_EXPOSURE;
  delete env.SOFTCLIP_AUTH_BASE_URL_MODE;
  console.log("[softclip] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[softclip] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@softclipai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[softclip] Command failed with code ${status.code}: pnpm --filter @softclipai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[softclip] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.SOFTCLIP_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[softclip] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[softclip] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@softclipai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[softclip] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@softclipai/server", serverScript, ...forwardedArgs],
