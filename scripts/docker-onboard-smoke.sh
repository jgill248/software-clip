...
IMAGE_NAME="${IMAGE_NAME:-softclip-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
SOFTCLIP_VERSION="${SOFTCLIP_VERSION:-latest}"
...
SOFTCLIP_DEPLOYMENT_MODE="${SOFTCLIP_DEPLOYMENT_MODE:-authenticated}"
SOFTCLIP_DEPLOYMENT_EXPOSURE="${SOFTCLIP_DEPLOYMENT_EXPOSURE:-private}"
SOFTCLIP_PUBLIC_URL="${SOFTCLIP_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@softclip.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-softclip-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_SOFTCLIP_VERSION=%q\n' "$SOFTCLIP_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e SOFTCLIP_DEPLOYMENT_MODE="$SOFTCLIP_DEPLOYMENT_MODE" \
      -e SOFTCLIP_DEPLOYMENT_EXPOSURE="$SOFTCLIP_DEPLOYMENT_EXPOSURE" \
      -e SOFTCLIP_PUBLIC_URL="$SOFTCLIP_PUBLIC_URL" \
      -e SOFTCLIP_HOME="/softclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "softclip@${SOFTCLIP_VERSION}" auth bootstrap-ceo --data-dir "$SOFTCLIP_HOME" --base-url "$SOFTCLIP_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$SOFTCLIP_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$SOFTCLIP_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$SOFTCLIP_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$SOFTCLIP_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$SOFTCLIP_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$SOFTCLIP_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg SOFTCLIP_VERSION="$SOFTCLIP_VERSION" \
...
echo "    Public URL: $SOFTCLIP_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $SOFTCLIP_DEPLOYMENT_MODE/$SOFTCLIP_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/softclip-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$SOFTCLIP_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $SOFTCLIP_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$SOFTCLIP_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $SOFTCLIP_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
