...
const DEFAULT_QUERY = "\"Co-Authored-By: Softclip <noreply@softclip.local>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/softclip-commit-metrics-cache.json");
...
const SOFTCLIP_EMAIL = "noreply@softclip.local";
const SOFTCLIP_NAME = "softclip";
...
function printHelp() {
  console.log(`Usage: tsx scripts/softclip-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === SOFTCLIP_EMAIL) || (displayName && displayName.toLowerCase() === SOFTCLIP_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Softclip commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "softclip-commit-metrics",
