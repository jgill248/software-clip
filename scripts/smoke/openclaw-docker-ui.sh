...
SOFTCLIP_HOST_PORT="${SOFTCLIP_HOST_PORT:-3100}"
SOFTCLIP_HOST_FROM_CONTAINER="${SOFTCLIP_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.softclip-openclaw.override.yml"
...
detect_softclip_base_url() {
  ...
  for candidate in "$SOFTCLIP_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${SOFTCLIP_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${SOFTCLIP_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
softclip_base_url="$(detect_softclip_base_url || true)"
...
Softclip URL for OpenClaw container:
EOF
  if [[ -n "$softclip_base_url" ]]; then
    cat <<EOF
  $softclip_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${SOFTCLIP_HOST_PORT}
  (Do not use http://127.0.0.1:${SOFTCLIP_HOST_PORT} inside the container.)
  If Softclip rejects the host, run on host machine:
    pnpm softclip allowed-hostname host.docker.internal
  Then restart Softclip and re-run this script.
EOF
  fi
  cat <<EOF
