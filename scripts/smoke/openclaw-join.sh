...
COMPANY_ID="${COMPANY_ID:-${SOFTCLIP_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-softclip-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-softclip-openclaw-smoke}"
...
Provide one of:
  SOFTCLIP_AUTH_HEADER=\"Bearer <board-token>\"
  SOFTCLIP_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Softclip OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.softclip.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
