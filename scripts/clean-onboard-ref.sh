...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Softclip data dir to use
  SOFTCLIP_HOST=127.0.0.1    Host passed to the onboarded server
  SOFTCLIP_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/softclip-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/softclip-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export SOFTCLIP_HOST="${SOFTCLIP_HOST:-127.0.0.1}"
export SOFTCLIP_PORT="${SOFTCLIP_PORT:-3100}"
export SOFTCLIP_OPEN_ON_LISTEN="${SOFTCLIP_OPEN_ON_LISTEN:-false}"
...
echo "SOFTCLIP_HOST: $SOFTCLIP_HOST"
echo "SOFTCLIP_PORT: $SOFTCLIP_PORT"
...
env \
  ...
  HOST="$SOFTCLIP_HOST" \
  PORT="$SOFTCLIP_PORT" \
  SOFTCLIP_OPEN_ON_LISTEN="$SOFTCLIP_OPEN_ON_LISTEN" \
  pnpm softclip onboard --yes --data-dir "$PC_DATA"
