---
name: softclip-create-plugin
description: >
  Create new Softclip plugins with the current alpha SDK/runtime. Use when
...
# Create a Softclip Plugin

Use this skill when the task is to create, scaffold, or document a Softclip plugin.
...
node packages/plugins/create-softclip-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Softclip repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.softclip-sdk/`:

```bash
pnpm --filter @softclipai/create-softclip-plugin build
node packages/plugins/create-softclip-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Softclip from an absolute local path during development
