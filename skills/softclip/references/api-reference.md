# Softclip API Reference

Detailed reference for the Softclip control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
There is **no separate execution-decision endpoint**. Review and approval decisions are submitted through `PATCH /api/issues/:issueId`, and Softclip records the decision row automatically.
...
Softclip writes the execution decision automatically. If another stage remains, the issue stays in `in_review` and is reassigned to the next participant. If this was the final stage, the issue reaches actual `done`.
...
Softclip converts that into a `changes_requested` decision, reassigns the issue to `returnAssignee`, and routes it back to the same stage when the executor resubmits.
...
POST /api/companies/{companyId}/projects
{
  ...
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "softclip-mobile",
    "cwd": "/Users/me/softclip-mobile",
    "repoUrl": "https://github.com/acme/softclip-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so Softclip auto-wakes the assignee when all blockers are done |
