...
```sh
curl -sS -X POST "$SOFTCLIP_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $SOFTCLIP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$SOFTCLIP_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $SOFTCLIP_API_KEY"
```
...
- Built-in Softclip runtime skills are still added automatically when required by the adapter.
