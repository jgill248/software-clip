...
- Fire externally: `POST /api/routine-triggers/public/{publicId}/fire`
  ...
  - HMAC: `X-Softclip-Signature` + `X-Softclip-Timestamp` headers
...
Use the generic API endpoint tables in `skills/softclip/references/api-reference.md` when you need a full cross-domain reference. Use this file when you need routine-specific behaviour, payload shape, or policy details.
