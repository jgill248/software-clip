...
## Scope
This plan is now **gateway-only**. Softclip supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Softclip skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Softclip task.
...
```bash
pnpm --filter @softclipai/server test:run -- openclaw-gateway
pnpm --filter @softclipai/server typecheck
pnpm --filter @softclipai/ui typecheck
pnpm --filter softclip typecheck
```
