...
Use when:
- You want Softclip to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Softclip server.
...
- softclipApiUrl (string, optional): absolute Softclip base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/softclip-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default softclip)

Standard outbound payload additions:
- softclip (object): standardized Softclip context added to every gateway agent request
- softclip.workspace (object, optional): resolved execution workspace for this run
- softclip.workspaces (array, optional): additional workspace hints Softclip exposed to the run
- softclip.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
