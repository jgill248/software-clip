...
Use when:
- You want Softclip to run Cursor Agent CLI locally as the agent runtime
...
- Softclip auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$softclip" and related skills on local runs.
- Softclip auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
