...
Use when:
- You want Softclip to run Pi (the AI coding agent) locally as the agent runtime
...
- Softclip requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/softclips/ and resumed with --session.
