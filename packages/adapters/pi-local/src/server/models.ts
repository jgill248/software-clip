...
import type { AdapterModel } from "@softclipai/adapter-utils";
import { asString, runChildProcess } from "@softclipai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.SOFTCLIP_PI_COMMAND === "string" &&
    process.env.SOFTCLIP_PI_COMMAND.trim().length > 0
      ? process.env.SOFTCLIP_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["SOFTCLIP_", "npm_", "NPM_"] as const;
