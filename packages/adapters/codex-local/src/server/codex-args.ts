import { asBoolean, asString, asStringArray } from "@softclipai/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Softclip will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
