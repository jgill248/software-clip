...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://softclip:softclip@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "softclip");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://softclip:softclip@127.0.0.1:${preferredPort}/softclip`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://softclip:softclip@127.0.0.1:${port}/softclip`,
  ...
  const adminConnectionString = `postgres://softclip:softclip@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "softclip");

  return {
    connectionString: `postgres://softclip:softclip@127.0.0.1:${selectedPort}/softclip`,
