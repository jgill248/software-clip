...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("softclip-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("softclip-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "softclip_restore_target",
      );
      const backupDir = createTempDir("softclip-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "softclip-test",
        });

        expect(result.backupFile).toMatch(/softclip-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("softclip-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Softclip database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- softclip statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
