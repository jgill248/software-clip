...
type GlobalBridge = typeof globalThis & {
  __softclipPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__softclipPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `Softclip plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Softclip plugin UI component "${name}" is not callable`);
}
