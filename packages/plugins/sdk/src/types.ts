/**
 * Core types for the Softclip plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@softclipai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @softclipai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Softclip object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Softclip UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Softclip domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Softclip domain event or a plugin-namespaced event.
   *
   ...
   */
  emit(name: string, productId: string, payload: unknown): Promise<void>;
}
 ...
 * This client resolves the reference through the Softclip secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Softclip secret provider (e.g. `"MY_API_KEY"`).
   *
...
export interface PluginProjectsClient {
   ...
   */
  list(input: { productId: string; limit?: number; offset?: number }): Promise<Project[]>;
   ...
   * Requires the `projects.read` capability.
   */
  get(projectId: string, productId: string): Promise<Project | null>;

  /**
   * List all workspaces attached to a project.
   *
   * @param projectId - UUID of the project
   * @param productId - UUID of the company that owns the project
   * @returns All workspaces for the project, ordered with primary first
   */
  listWorkspaces(projectId: string, productId: string): Promise<PluginWorkspace[]>;
   ...
   */
  getPrimaryWorkspace(projectId: string, productId: string): Promise<PluginWorkspace | null>;
   ...
   * @param productId - UUID of the company that owns the issue
   * @returns The primary workspace for the issue's project, or `null` if
   *   the issue has no project or the project has no workspace
   *
   * @see PLUGIN_SPEC.md §20 — Local Tooling
   */
  getWorkspaceForIssue(issueId: string, productId: string): Promise<PluginWorkspace | null>;
}
...
export interface PluginCompaniesClient {
   ...
   */
  get(productId: string): Promise<Company | null>;
}
...
export interface PluginIssueDocumentsClient {
   ...
   */
  list(issueId: string, productId: string): Promise<IssueDocumentSummary[]>;
   ...
   * @param productId - UUID of the company
   */
  get(issueId: string, key: string, productId: string): Promise<IssueDocument | null>;
   ...
   * @param productId - UUID of the company
   */
  delete(issueId: string, key: string, productId: string): Promise<void>;
}
...
export interface PluginIssuesClient {
  ...
  get(issueId: string, productId: string): Promise<Issue | null>;
  create(input: {
    productId: string;
    projectId?: string;
    goalId?: string;
  ...
  listComments(issueId: string, productId: string): Promise<IssueComment[]>;
  createComment(
    issueId: string,
    body: string,
    productId: string,
...
export interface PluginAgentsClient {
  list(input: { productId: string; status?: Agent["status"]; limit?: number; offset?: number }): Promise<Agent[]>;
  get(agentId: string, productId: string): Promise<Agent | null>;
  /** Pause an agent. Throws if agent is terminated or not found. Requires `agents.pause`. */
  pause(agentId: string, productId: string): Promise<Agent>;
  /** Resume a paused agent (sets status to idle). Throws if terminated, pending_approval, or not found. Requires `agents.resume`. */
  resume(agentId: string, productId: string): Promise<Agent>;
  ...
  invoke(agentId: string, productId: string, opts: { prompt: string; reason?: string }): Promise<{ runId: string }>;
...
export interface PluginAgentSessionsClient {
  ...
  create(agentId: string, productId: string, opts?: {
    taskKey?: string;
    reason?: string;
  }): Promise<AgentSession>;

  /** List active sessions for an agent owned by this plugin. Requires `agent.sessions.list`. */
  list(agentId: string, productId: string): Promise<AgentSession[]>;
   ...
   */
  sendMessage(sessionId: string, productId: string, opts: {
    prompt: string;
    reason?: string;
  ...
  close(sessionId: string, productId: string): Promise<void>;
}
...
export interface PluginGoalsClient {
  ...
  get(goalId: string, productId: string): Promise<Goal | null>;
  create(input: {
    productId: string;
    title: string;
    description?: string;
 ...
 * real time via SSE.
 *
 * Streams are scoped to `(pluginId, channel, productId)`. Multiple UI clients
 * can subscribe to the same channel concurrently.
 *
 * @example
 * ```ts
 * // Worker: stream chat tokens to the UI
 * ctx.streams.open("chat", productId);
 ...
 * ```
 *
 * @see usePluginStream in `@softclipai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@softclipai/plugin-sdk";
 *
 ...
 *       const state = await ctx.state.get({ scopeKind: "company", scopeId: String(productId), stateKey: "last-sync" });
