...
import type { SoftclipPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@softclipai/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: SoftclipPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@softclipai/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
