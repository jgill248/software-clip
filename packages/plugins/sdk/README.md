# `@softclipai/plugin-sdk`

Official TypeScript SDK for Softclip plugin authors.

- **Worker SDK:** `@softclipai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@softclipai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@softclipai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@softclipai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@softclipai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@softclipai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@softclipai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@softclipai/plugin-sdk/ui/hooks` | Hooks only |
| `@softclipai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@softclipai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@softclipai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@softclipai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@softclipai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@softclipai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @softclipai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Softclip app. They can call ordinary Softclip HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Softclip instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@softclipai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@softclipai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Softclip information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@softclipai/plugin-sdk`.
...
import { usePluginAction, type PluginBridgeError } from "@softclipai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@softclipai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@softclipai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@softclipai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@softclipai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@softclipai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@softclipai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@softclipai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@softclipai/plugin-sdk/bundlers";
...
```bash
softclip-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@softclipai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__softclip__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__softclip__/events` streams `reload` SSE events on UI build changes
