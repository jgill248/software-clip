...
Example Softclip plugin that demonstrates:
...
```bash
pnpm --filter @softclipai/plugin-file-browser-example build
pnpm softclip plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm softclip plugin uninstall softclip-file-browser-example --force
```
...
- Optional: use `softclip-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
