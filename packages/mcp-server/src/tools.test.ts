...
describe("softclip MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("softclipUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-Softclip-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("softclipListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("softclipCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("softclipUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("softclipCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("softclipApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
