...
/**
 * Zod schema for {@link SoftclipPluginManifestV1} — the complete runtime
 ...
 * | `minimumSoftclipVersion`| string?    | legacy alias of `minimumHostVersion`         |
 ...
 *
 * @see PLUGIN_SPEC.md §10.1 — Manifest shape
 * @see {@link SoftclipPluginManifestV1} — the inferred TypeScript type
 */
export const pluginManifestV1Schema = z.object({
  ...
  minimumSoftclipVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumSoftclipVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumSoftclipVersion
    && manifest.minimumHostVersion !== manifest.minimumSoftclipVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumSoftclipVersion must match when both are declared",
