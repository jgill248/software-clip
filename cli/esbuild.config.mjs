/**
 * esbuild configuration for building the softclip CLI for npm.
 *
 * Bundles all workspace packages (@softclipai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @softclipai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@softclipai/")) {
      externals.add(name);
    }
  }
