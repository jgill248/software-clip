...
const JWT_SECRET_ENV_KEY = "SOFTCLIP_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Softclip CLI commands",
...
export function resolveSoftclipEnvFile(configPath?: string): string {
...
export function loadSoftclipEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeSoftclipEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readSoftclipEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeSoftclipEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeSoftclipEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readSoftclipEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeSoftclipEnvEntries(next, filePath);
  return next;
}
