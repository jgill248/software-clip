...
import { softclipConfigSchema, type SoftclipConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".softclip", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  if (process.env.SOFTCLIP_CONFIG) return path.resolve(process.env.SOFTCLIP_CONFIG);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveSoftclipInstanceId());
}
...
export function readConfig(configPath?: string): SoftclipConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = softclipConfigSchema.safeParse(migrated);
  if (!parsed.success) {
