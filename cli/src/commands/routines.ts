...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://softclip:softclip@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "softclip");
      const connectionString = `postgres://softclip:softclip@127.0.0.1:${embeddedHandle.port}/softclip`;
...
export async function disableAllRoutinesInConfig(
  ...
  const productId =
    ...
    ?? nonEmpty(process.env.SOFTCLIP_COMPANY_ID)
    ?? null;
  if (!productId) {
    throw new Error("Company ID is required. Pass --company-id or set SOFTCLIP_COMPANY_ID.");
  }
