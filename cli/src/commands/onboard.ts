...
type OnboardDefaults = Pick<SoftclipConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or SOFTCLIP_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "SOFTCLIP_PUBLIC_URL",
  "DATABASE_URL",
  "SOFTCLIP_DB_BACKUP_ENABLED",
  "SOFTCLIP_DB_BACKUP_INTERVAL_MINUTES",
  "SOFTCLIP_DB_BACKUP_RETENTION_DAYS",
  "SOFTCLIP_DB_BACKUP_DIR",
  "SOFTCLIP_DEPLOYMENT_MODE",
  "SOFTCLIP_DEPLOYMENT_EXPOSURE",
  "SOFTCLIP_BIND",
  "SOFTCLIP_BIND_HOST",
  "SOFTCLIP_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "SOFTCLIP_ALLOWED_HOSTNAMES",
  "SOFTCLIP_AUTH_BASE_URL_MODE",
  "SOFTCLIP_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "SOFTCLIP_STORAGE_PROVIDER",
  "SOFTCLIP_STORAGE_LOCAL_DIR",
  "SOFTCLIP_STORAGE_S3_BUCKET",
  "SOFTCLIP_STORAGE_S3_REGION",
  "SOFTCLIP_STORAGE_S3_ENDPOINT",
  "SOFTCLIP_STORAGE_S3_PREFIX",
  "SOFTCLIP_STORAGE_S3_FORCE_PATH_STYLE",
  "SOFTCLIP_SECRETS_PROVIDER",
  "SOFTCLIP_SECRETS_STRICT_MODE",
  "SOFTCLIP_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<SoftclipConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveSoftclipInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.SOFTCLIP_PUBLIC_URL?.trim() ||
      process.env.SOFTCLIP_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.SOFTCLIP_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.SOFTCLIP_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.SOFTCLIP_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.SOFTCLIP_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.SOFTCLIP_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.SOFTCLIP_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.SOFTCLIP_ALLOWED_HOSTNAMES
    ? process.env.SOFTCLIP_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.SOFTCLIP_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.SOFTCLIP_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.SOFTCLIP_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.SOFTCLIP_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.SOFTCLIP_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.SOFTCLIP_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.SOFTCLIP_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.SOFTCLIP_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.SOFTCLIP_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.SOFTCLIP_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.SOFTCLIP_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.SOFTCLIP_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.SOFTCLIP_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.SOFTCLIP_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.SOFTCLIP_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "SOFTCLIP_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.SOFTCLIP_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "SOFTCLIP_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.SOFTCLIP_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "SOFTCLIP_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<SoftclipConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" softclip onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveSoftclipInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: SoftclipConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Softclip install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("softclip configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("SOFTCLIP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.SOFTCLIP_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("SOFTCLIP_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("SOFTCLIP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: SOFTCLIP_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("softclip run")}`,
        `Reconfigure later: ${pc.cyan("softclip configure")}`,
        `Diagnose setup: ${pc.cyan("softclip doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.SOFTCLIP_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Softclip setup is ready.");
    return;
  }
  ...
  let llm: SoftclipConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@softclipai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `softclip doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("softclip run")}`,
        `Then: ${pc.cyan("softclip auth bootstrap-product-owner")}`,
      ].join("\n"),
    );
  }
