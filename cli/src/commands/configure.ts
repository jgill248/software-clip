...
function defaultConfig(): SoftclipConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" softclip configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `softclip onboard` first.");
    p.outro("");
    return;
  }

  let config: SoftclipConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because SOFTCLIP_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
