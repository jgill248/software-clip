...
export async function resolveFeedbackCompanyId(
  ...
  if (!productId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SOFTCLIP_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return productId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Softclip Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Softclip Feedback Export")));
