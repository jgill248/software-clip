...
import { ApiRequestError, SoftclipApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Softclip config file")
    .option("-d, --data-dir <path>", "Softclip data directory root (isolates state from ~/.softclip)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Softclip API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SOFTCLIP_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.SOFTCLIP_API_KEY?.trim() ||
  ...
  const productId =
    ...
    process.env.SOFTCLIP_COMPANY_ID?.trim() ||
    profile.productId;

  if (opts?.requireCompany && !productId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SOFTCLIP_COMPANY_ID, or set context profile productId via `softclip context set`.",
    );
  }

  const api = new SoftclipApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SOFTCLIP_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SOFTCLIP_SERVER_PORT || "");
