...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveSoftclipInstanceId(opts.instance);
  process.env.SOFTCLIP_INSTANCE_ID = instanceId;

  const homeDir = resolveSoftclipHomeDir();
  ...
  p.intro(pc.bgCyan(pc.black(" softclip run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("softclip onboard")} once, then retry ${pc.cyan("softclip run")}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.SOFTCLIP_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@softclipai/server/src/index.ts")) {
    process.env.SOFTCLIP_UI_DEV_MIDDLEWARE = "true";
  }
}
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Softclip dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @softclipai/server package
  try {
    const mod = await import("@softclipai/server");
    return await startServerFromModule(mod, "@softclipai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@softclipai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Softclip server entrypoint.\n` +
          `Tried: ${devEntry}, @softclipai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Softclip server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: SoftclipConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Softclip server entrypoint did not export startServer(): ${label}`);
  }
