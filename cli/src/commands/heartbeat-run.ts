...
import type { Agent, HeartbeatRun, HeartbeatRunEvent, HeartbeatRunStatus } from "@softclipai/shared";
  ...
  while (true) {
    ...
    for (const event of Array.isArray(events) ? (events as HeartbeatRunEventRecord[]) : []) {
      ...
      const runList = (await api.get<(HeartbeatRun | null)[]>(
        `/api/companies/${agent.productId}/heartbeat-runs?agentId=${agent.id}`,
      )) || [];
