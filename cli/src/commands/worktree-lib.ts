...
export const DEFAULT_WORKTREE_HOME = "~/.softclip-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".softclip");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: SoftclipConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "softclip",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    SOFTCLIP_HOME: paths.homeDir,
    SOFTCLIP_INSTANCE_ID: paths.instanceId,
    SOFTCLIP_CONFIG: paths.configPath,
    SOFTCLIP_CONTEXT: paths.contextPath,
    SOFTCLIP_IN_WORKTREE: "true",
    ...(branding?.name ? { SOFTCLIP_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { SOFTCLIP_WORKTREE_COLOR: branding.color } : {}),
  };
}
