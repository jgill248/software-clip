...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "softclip-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".softclip", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "softclip-onboard-fresh-"));
  return path.join(root, ".softclip", "config.json");
}
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.SOFTCLIP_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as SoftclipConfig;
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.SOFTCLIP_TAILNET_BIND_HOST = "100.64.0.8";
  ...
  it("keeps tailnet quickstart on loopback until tailscale is available", async () => {
    const configPath = createFreshConfigPath();
    delete process.env.SOFTCLIP_TAILNET_BIND_HOST;
  ...
  it("ignores deployment env overrides during --yes quickstart", async () => {
    const configPath = createFreshConfigPath();
    process.env.SOFTCLIP_DEPLOYMENT_MODE = "authenticated";
