...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "softclip-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.SOFTCLIP_HOME = path.join(root, "home");
    process.env.SOFTCLIP_INSTANCE_ID = "telemetry-test";
