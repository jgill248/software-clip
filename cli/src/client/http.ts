...
export class SoftclipApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-softclip-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Softclip server is not running, the configured URL is wrong, or the request is being blocked before it reaches Softclip.",
    "",
    "Try:",
    "- Start Softclip with `pnpm dev` or `pnpm softclip run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Softclip is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`SOFTCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
