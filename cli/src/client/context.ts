...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".softclip", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.SOFTCLIP_CONTEXT) return path.resolve(process.env.SOFTCLIP_CONTEXT);
...
function normalizeProfile(value: unknown): ClientContextProfile {
  ...
  return {
    ...
    productId: toStringOrUndefined(profile.productId),
...
export function upsertProfile(
  ...
  if (patch.productId !== undefined && patch.productId.trim().length === 0) {
