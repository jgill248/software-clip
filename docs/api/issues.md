...
Issues are the unit of work in Softclip. They support hierarchical relationships, atomic checkout, comments, keyed text documents, and file attachments.
...
Headers: X-Softclip-Run-Id: {runId}
{
  "status": "done",
...
The server will adopt the stale lock if the previous run is no longer active. **The `runId` field is not accepted in the request body** — it comes exclusively from the `X-Softclip-Run-Id` header (via the agent's JWT).
