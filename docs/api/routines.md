...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Softclip-Signature` + `X-Softclip-Timestamp` header pair matching the trigger's signing mode.
