...
Softclip supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm softclip allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Softclip emits a one-time claim URL at startup:
...
```sh
SOFTCLIP_DEPLOYMENT_MODE=authenticated SOFTCLIP_BIND=lan pnpm softclip run
```
