...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Softclip saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
# Installed CLI
softclip feedback report

# Point to a different server or company
pnpm softclip feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm softclip feedback report --payloads
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Softclip feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Softclip envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm softclip feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Endpoint: there is no default backend — feedback export is disabled unless `feedbackExportBackendUrl` is configured
