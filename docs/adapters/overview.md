...
summary: What adapters are and how they connect agents to Softclip
---

Adapters are the bridge between Softclip's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Softclip:
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-softclip-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to Softclip's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `softclip run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the Softclip web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
