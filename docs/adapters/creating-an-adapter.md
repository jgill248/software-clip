...
Build a custom adapter to connect Softclip to any agent runtime.
...
|---|---|---|
| Source | Inside `softclip-fork` | Separate npm package |
| Distribution | Ships with Softclip | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Softclip's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@softclipai/adapter-utils/server-utils`
2. Build environment with `buildSoftclipEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@softclipai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildSoftclipEnv(agent)` | `@softclipai/adapter-utils/server-utils` | Inject `SOFTCLIP_*` env vars |
| `renderTemplate(tpl, data)` | `@softclipai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@softclipai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@softclipai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Softclip's source:
...
`format-event.ts` — pretty-prints stdout for `softclip run --watch` using `picocolors`.
...
With these flags set, the Softclip UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Softclip source changes required.
...
Make Softclip skills discoverable to your agent runtime without writing to the agent's working directory:
