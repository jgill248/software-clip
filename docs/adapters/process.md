...
- Running a Python script that calls the Softclip API
...
1. Softclip spawns the configured command as a child process
2. Standard Softclip environment variables are injected (`SOFTCLIP_AGENT_ID`, `SOFTCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Softclip API and perform work.
