...
summary: Build, package, and distribute adapters as plugins without modifying Softclip source
---

Softclip supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Softclip's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `softclip-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Softclip | Published to npm or linked via `file:` |
| Updates | Requires Softclip release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@softclipai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `softclip.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@softclipai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Softclip vars injected
  const env = buildSoftclipEnv(agent);
...
#### Available Helpers from `@softclipai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildSoftclipEnv(agent)` | Inject `SOFTCLIP_*` environment variables |
...
```sh
# Via the Softclip UI
# Settings → Adapters → Install from npm → "my-softclip-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-softclip-adapter"}'
```
...
Local adapters are symlinked into Softclip's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.softclip/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-softclip-adapter",
...
```ts
import type { AdapterSessionCodec } from "@softclipai/adapter-utils";
...
Other Softclip users can then install your adapter by package name from the UI or API.
