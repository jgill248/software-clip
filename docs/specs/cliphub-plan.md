# ClipHub: Marketplace for Softclip Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Softclip configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Softclip-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Softclip company (agents, projects, governance) |
...
A complete Softclip company configuration:
...
Single-agent configurations designed to plug into a Softclip org:
...
Portable skill files that any Softclip agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Softclip's skill loading system
...
interface Listing {
  ...
  softclipVersionMin: string;     // Minimum Softclip version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Softclip company
...
| `POST` | `/api/listings/:id/install` | Install to Softclip company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Softclip company (or create new)
  → ClipHub API calls Softclip API to:
  ...
  → Redirect to Softclip dashboard with new team running
```
...
### 5.3 Creator: Export from Softclip → Publish

```
Running Softclip company → "Export as Blueprint" (CLI or UI)
  → Softclip exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Softclip brand blue/purple)
...
| Install flow | One-click deploy to Softclip company |
...
{
  "targetCompanyId": "uuid",         // Existing Softclip company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Softclip
- **Backend**: Node.js API (or extend Softclip server)
- **Database**: Postgres (can share Softclip's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Softclip auth (or OAuth2)

### 9.2 Integration with Softclip

ClipHub can be:
- **Option A**: A separate app that calls Softclip's API to install blueprints
- **Option B**: A built-in section of the Softclip UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Softclip UI and API.
...
- [ ] Install flow: blueprint → Softclip company
...
- [ ] "Export from Softclip" CLI command
