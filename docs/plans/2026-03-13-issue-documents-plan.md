...
Add first-class **documents** to Softclip as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Softclip workflow/docs.
...
Update the Softclip agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/softclip/SKILL.md`
