...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Softclip.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.softclip.yaml`
...
- Softclip must not require extra top-level fields for skill validity
- Softclip-specific extensions must live under `metadata.softclip` or `metadata.sources`
...
For Softclip, the preferred fidelity extension is:
...
- issue/task Softclip-only metadata
...
- Softclip tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Softclip shape:
...
Additional rules for Softclip exporters:
...
- omit empty and default-valued Softclip fields when possible
...
- Softclip exporters should emit `.softclip.yaml` as a sidecar by default
...
Softclip can map this spec to its runtime model like this:
...
- Softclip extension:
  - `.softclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Softclip-specific fidelity

Inline Softclip-only metadata that must live inside a shared markdown file should use:

- `metadata.softclip`

That keeps the base format broader than Softclip.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Softclip.

## 20. Cutover

Softclip should cut over to this markdown-first package model as the primary portability format.

`softclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Softclip, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
