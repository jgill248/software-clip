...
Audience: Operators setting up and running agents in Softclip

## 1. What this system does

Agents in Softclip do not run continuously.  
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-softclip-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Softclip assumes the CLI is already installed and authenticated on the host machine.
...
Softclip stores session IDs for resumable adapters.
...
Softclip pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Softclip surfaces this as a warning in environment tests, not a hard error.
