...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Softclip.

Softclip now presents this as a workspace-command model:
...
- Softclip does not automatically start or stop these workspace services as part of issue execution.
- Softclip also does not automatically restart workspace services on server boot.
...
2. Softclip realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Softclip persists execution-workspace metadata such as paths, refs, and provisioning settings.
