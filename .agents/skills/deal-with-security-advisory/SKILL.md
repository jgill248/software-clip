...
description: >
  Handle a GitHub Security Advisory response for Softclip, including
...
```
gh api repos/jgill248/software-clip/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/jgill248/software-clip/security-advisories/{{ghsaId}}/forks
...
```
# Clone the private fork somewhere outside ~/softclip
...
**Do not edit `~/softclip`** — the dev server is running off the `~/softclip` master branch and we don't want to touch it. All work happens in the private fork clone.
...
```
gh api --method POST \
  repos/jgill248/software-clip/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/jgill248/software-clip/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/jgill248/software-clip/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/jgill248/software-clip/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo jgill248/software-clip
...
* The published advisory URL: `https://github.com/jgill248/software-clip/security/advisories/{{ghsaId}}`
* The release URL
