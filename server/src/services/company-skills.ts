...
function uniqueImportedSkillKey(productId: string, baseSlug: string, usedKeys: Set<string>) {
  const initial = `company/${productId}/${baseSlug}`;
  if (!usedKeys.has(initial)) return initial;
  let attempt = 2;
  let candidate = `company/${productId}/${baseSlug}-${attempt}`;
  while (usedKeys.has(candidate)) {
    attempt += 1;
    candidate = `company/${productId}/${baseSlug}-${attempt}`;
  }
  return candidate;
}
...
function readInlineSkillImports(productId: string, files: Record<string, string>): ImportedSkill[] {
  ...
  for (const skillPath of skillPaths) {
    ...
    imports[imports.length - 1]!.key = deriveCanonicalSkillKey(productId, imports[imports.length - 1]!);
  }
...
export async function readLocalSkillImportFromDirectory(
  ...
  return {
    key: deriveCanonicalSkillKey(productId, {
      slug,
...
async function readLocalSkillImports(productId: string, sourcePath: string): Promise<ImportedSkill[]> {
  ...
  for (const skillPath of skillPaths) {
    ...
    const inventory = allFiles
      ...
      .sort((left, right) => left.path.localeCompare(right.path));
    const imported = await readLocalSkillImportFromDirectory(productId, path.join(root, skillDir));
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (skill.sourceType === "local_path") {
    const managedRoot = resolveManagedSkillsRoot(skill.productId);
...
export function companySkillService(db: Db) {
  ...
  async function ensureBundledSkills(productId: string) {
    for (const skillsRoot of resolveBundledSkillsRoot()) {
      const stats = await fs.stat(skillsRoot).catch(() => null);
      if (!stats?.isDirectory()) continue;
      const bundledSkills = await readLocalSkillImports(productId, skillsRoot)
      ...
      return upsertImportedSkills(productId, bundledSkills);
    }
    return [];
  }

  async function pruneMissingLocalPathSkills(productId: string) {
    const rows = await db
      .select()
      .from(companySkills)
    ...
    for (const skill of skills) {
      ...
      await fs.rm(resolveRuntimeSkillMaterializedPath(productId, skill), { recursive: true, force: true });
    }
  }

  async function ensureSkillInventoryCurrent(productId: string) {
    const existingRefresh = skillInventoryRefreshPromises.get(productId);
    if (existingRefresh) {
    ...
    const refreshPromise = (async () => {
      await ensureBundledSkills(productId);
      await pruneMissingLocalPathSkills(productId);
    })();

    skillInventoryRefreshPromises.set(productId, refreshPromise);
    try {
      await refreshPromise;
    } finally {
      if (skillInventoryRefreshPromises.get(productId) === refreshPromise) {
        skillInventoryRefreshPromises.delete(productId);
      }
    }
  }

  async function list(productId: string): Promise<CompanySkillListItem[]> {
    await ensureSkillInventoryCurrent(productId);
    const rows = await db
      .select({
        id: companySkills.id,
        productId: companySkills.productId,
    ...
    const agentRows = await agents.list(productId);
  ...
  async function listFull(productId: string): Promise<CompanySkill[]> {
    await ensureSkillInventoryCurrent(productId);
    const rows = await db
      .select()
      .from(companySkills)
  ...
  async function getByKey(productId: string, key: string) {
    const row = await db
      .select()
      .from(companySkills)
      .where(and(eq(companySkills.productId, productId), eq(companySkills.key, key)))
      .then((rows) => rows[0] ?? null);
    return row ? toCompanySkill(row) : null;
  }

  async function usage(productId: string, key: string): Promise<CompanySkillUsageAgent[]> {
      ...
      desiredAgents.map(async (agent) => {
        ...
        if (!adapter?.listSkills) {
          ...
          try {
            ...
            const runtimeSkillEntries = await listRuntimeSkillEntries(agent.productId);
  ...
  async function detail(productId: string, id: string): Promise<CompanySkillDetail | null> {
    await ensureSkillInventoryCurrent(productId);
    const skill = await getById(id);
    if (!skill || skill.productId !== productId) return null;
    const usedByAgents = await usage(productId, skill.key);
    return enrichSkill(skill, usedByAgents.length, usedByAgents);
  }

  async function updateStatus(productId: string, skillId: string): Promise<CompanySkillUpdateStatus | null> {
    await ensureSkillInventoryCurrent(productId);
  ...
  async function readFile(productId: string, skillId: string, relativePath: string): Promise<CompanySkillFileDetail | null> {
    await ensureSkillInventoryCurrent(productId);
    const skill = await getById(skillId);
    if (!skill || skill.productId !== productId) return null;
  ...
  async function createLocalSkill(productId: string, input: CompanySkillCreateRequest): Promise<CompanySkill> {
    const slug = normalizeSkillSlug(input.slug ?? input.name) ?? "skill";
    const managedRoot = resolveManagedSkillsRoot(productId);
    ...
    const parsed = parseFrontmatterMarkdown(markdown);
    const imported = await upsertImportedSkills(productId, [{
      key: `company/${productId}/${slug}`,
      slug,
  ...
  async function updateFile(productId: string, skillId: string, relativePath: string, content: string): Promise<CompanySkillFileDetail> {
    await ensureSkillInventoryCurrent(productId);
    const skill = await getById(skillId);
    if (!skill || skill.productId !== productId) throw notFound("Skill not found");
    ...
    const detail = await readFile(productId, skillId, normalizedPath);
    if (!detail) throw notFound("Skill file not found");
    return detail;
  }

  async function installUpdate(productId: string, skillId: string): Promise<CompanySkill | null> {
    await ensureSkillInventoryCurrent(productId);
    const skill = await getById(skillId);
    if (!skill || skill.productId !== productId) return null;

    const status = await updateStatus(productId, skillId);
    if (!status?.supported) {
    ...
    const result = await readUrlSkillImports(productId, skill.sourceLocator, skill.slug);
    ...
    const imported = await upsertImportedSkills(productId, [matching]);
  ...
  async function scanProjectWorkspaces(
    ...
    const projectRows = input.projectIds?.length
      ? await projects.listByIds(productId, input.projectIds)
      : await projects.list(productId);
    ...
    const availableSkills = await listFull(productId);
    ...
    for (const target of scanTargets) {
      ...
      for (const directory of directories) {
        ...
        try {
          nextSkill = await readLocalSkillImportFromDirectory(productId, directory.skillDir, {
        ...
        if (existingByKey) {
          ...
          const persisted = (await upsertImportedSkills(productId, [nextSkill]))[0];
  ...
  async function materializeCatalogSkillFiles(
    ...
    const catalogRoot = path.resolve(resolveManagedSkillsRoot(productId), "__catalog__");
  ...
  async function materializeRuntimeSkillFiles(productId: string, skill: CompanySkill) {
    const runtimeRoot = path.resolve(resolveManagedSkillsRoot(productId), "__runtime__");
    const skillDir = path.resolve(runtimeRoot, buildSkillRuntimeName(skill.key, skill.slug));
    await fs.rm(skillDir, { recursive: true, force: true });
    await fs.mkdir(skillDir, { recursive: true });

    for (const entry of skill.fileInventory) {
      const detail = await readFile(productId, skill.id, entry.path).catch(() => null);
      if (!detail) continue;
      const targetPath = path.resolve(skillDir, entry.path);
      await fs.mkdir(path.dirname(targetPath), { recursive: true });
  ...
  function resolveRuntimeSkillMaterializedPath(productId: string, skill: CompanySkill) {
    const runtimeRoot = path.resolve(resolveManagedSkillsRoot(productId), "__runtime__");
    ...
    for (const skill of skills) {
      ...
      if (!source) {
        source = options.materializeMissing === false
          ? resolveRuntimeSkillMaterializedPath(productId, skill)
          : await materializeRuntimeSkillFiles(productId, skill).catch(() => null);
      }
      if (!source) continue;
  ...
  async function importPackageFiles(
    ...
    await ensureSkillInventoryCurrent(productId);
    const normalizedFiles = normalizePackageFileMap(files);
    const importedSkills = readInlineSkillImports(productId, normalizedFiles);
    if (importedSkills.length === 0) return [];

    for (const skill of importedSkills) {
      if (skill.sourceType !== "catalog") continue;
      const materializedDir = await materializeCatalogSkillFiles(productId, skill, normalizedFiles);
      if (materializedDir) {
    ...
    const existingSkills = await listFull(productId);
    ...
    for (const importedSkill of importedSkills) {
      ...
      const renamedKey = uniqueImportedSkillKey(productId, renamedSlug, usedKeys);
    ...
    const persisted = await upsertImportedSkills(productId, toPersist);
  ...
  async function upsertImportedSkills(productId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    const out: CompanySkill[] = [];
    for (const skill of imported) {
      const existing = await getByKey(productId, skill.key);
  ...
  async function importFromSource(productId: string, source: string): Promise<CompanySkillImportResult> {
    await ensureSkillInventoryCurrent(productId);
    const parsed = parseSkillImportSourceInput(source);
    const local = !/^https?:\/\//i.test(parsed.resolvedSource);
    const { skills, warnings } = local
      ? {
        skills: (await readLocalSkillImports(productId, parsed.resolvedSource))
          .filter((skill) => !parsed.requestedSkillSlug || skill.slug === parsed.requestedSkillSlug),
        warnings: parsed.warnings,
      }
      : await readUrlSkillImports(productId, parsed.resolvedSource, parsed.requestedSkillSlug)
        .then((result) => ({
    ...
    if (parsed.originalSkillsShUrl) {
      for (const skill of filteredSkills) {
        ...
        skill.key = deriveCanonicalSkillKey(productId, skill);
      }
    }
    const imported = await upsertImportedSkills(productId, filteredSkills);
    return { imported, warnings };
  }

  async function deleteSkill(productId: string, skillId: string): Promise<CompanySkill | null> {
    const row = await db
      .select()
      .from(companySkills)
      .where(and(eq(companySkills.id, skillId), eq(companySkills.productId, productId)))
      .then((rows) => rows[0] ?? null);
    if (!row) return null;
  ...
  return {
    ...
    getById,
    getByKey,
    resolveRequestedSkillKeys: async (productId: string, requestedReferences: string[]) => {
      const skills = await listFull(productId);
