...
import { projects, projectGoals, goals, projectWorkspaces, workspaceRuntimeServices } from "@softclipai/db";
import {
  PROJECT_COLORS,
  deriveProjectUrlKey,
  hasNonAsciiContent,
...
async function syncGoalLinks(db: Db, projectId: string, productId: string, goalIds: string[]) {
  ...
  if (goalIds.length > 0) {
    await db.insert(projectGoals).values(
      goalIds.map((goalId) => ({ projectId, goalId, productId })),
    );
  }
}
...
async function ensureSinglePrimaryWorkspace(
  ...
  await dbOrTx
    ...
    .where(
      and(
        eq(projectWorkspaces.productId, input.productId),
...
export function projectService(db: Db) {
  return {
    list: async (productId: string): Promise<ProjectWithGoals[]> => {
      const rows = await db.select().from(projects).where(eq(projects.productId, productId));
      const withGoals = await attachGoals(db, rows);
      return attachWorkspaces(db, withGoals);
    },

    listByIds: async (productId: string, ids: string[]): Promise<ProjectWithGoals[]> => {
      const dedupedIds = [...new Set(ids)];
      if (dedupedIds.length === 0) return [];
      const rows = await db
        .select()
        .from(projects)
        .where(and(eq(projects.productId, productId), inArray(projects.id, dedupedIds)));
      const withGoals = await attachGoals(db, rows);
    ...
    create: async (
      productId: string,
      data: Omit<typeof projects.$inferInsert, "productId"> & { goalIds?: string[] },
      ...
      if (!projectData.color) {
        const existing = await db.select({ color: projects.color }).from(projects).where(eq(projects.productId, productId));
      ...
      const row = await db
        .insert(projects)
        .values({ ...projectData, goalId: legacyGoalId, productId })
        .returning()
        .then((rows) => rows[0]);

      if (ids && ids.length > 0) {
        await syncGoalLinks(db, row.id, productId, ids);
      }
    ...
    update: async (
      ...
      const existingProject = await db
        .select({ id: projects.id, productId: projects.productId, name: projects.name })
        .from(projects)
        .where(eq(projects.id, id))
        .then((rows) => rows[0] ?? null);
      ...
      if (projectData.name !== undefined) {
        ...
        if (existingShortname !== nextShortname) {
          const existingProjects = await db
            .select({ id: projects.id, name: projects.name })
            .from(projects)
            .where(eq(projects.productId, existingProject.productId));
          projectData.name = resolveProjectNameForUniqueShortname(projectData.name, existingProjects, {
            excludeProjectId: id,
          });
        }
      ...
      if (ids !== undefined) {
        await syncGoalLinks(db, id, row.productId, ids);
      }
      ...
      const created = await db.transaction(async (tx) => {
        if (shouldBePrimary) {
          await tx
            ...
            .where(
              and(
                eq(projectWorkspaces.productId, project.productId),
    ...
    updateWorkspace: async (
      ...
      const updated = await db.transaction(async (tx) => {
        ...
        const hasPrimary = await tx
          ...
          .where(
            and(
              eq(projectWorkspaces.productId, row.productId),
    ...
    resolveByReference: async (productId: string, reference: string) => {
      ...
      if (isUuidLike(raw)) {
        const row = await db
          .select({ id: projects.id, productId: projects.productId, name: projects.name })
          .from(projects)
          .where(and(eq(projects.id, raw), eq(projects.productId, productId)))
          .then((rows) => rows[0] ?? null);
        if (!row) return { project: null, ambiguous: false } as const;
        return {
          project: { id: row.id, productId: row.productId, urlKey: deriveProjectUrlKey(row.name, row.id) },
          ambiguous: false,
        } as const;
      }
      ...
      if (matches.length === 1) {
        ...
        return {
          project: { id: match.id, productId: match.productId, urlKey: deriveProjectUrlKey(match.name, match.id) },
          ambiguous: false,
        } as const;
      }
      if (matches.length > 1) {
