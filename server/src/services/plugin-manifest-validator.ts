 ...
 */
import { pluginManifestV1Schema } from "@softclipai/shared";
import type { SoftclipPluginManifestV1 } from "@softclipai/shared";
import { PLUGIN_API_VERSION } from "@softclipai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): SoftclipPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as SoftclipPluginManifestV1,
        };
      }
