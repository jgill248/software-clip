...
import type { SoftclipPluginManifestV1 } from "@softclipai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __softclip_exports: module.exports,
      __softclip_module: module,
      __softclip_require: requireInSandbox,
      __softclip_filename: realPath,
      __softclip_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__softclip_exports, __softclip_module, __softclip_require, __softclip_filename, __softclip_dirname)`;
