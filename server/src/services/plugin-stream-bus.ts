...
/**
 * Composite key for stream subscriptions: pluginId:channel:productId
 */
function streamKey(pluginId: string, channel: string, productId: string): string {
  return `${pluginId}:${channel}:${productId}`;
}

export interface PluginStreamBus {
  /**
   * Subscribe to stream events for a specific (pluginId, channel, productId).
  ...
  /**
   * Publish an event to all subscribers of (pluginId, channel, productId).
...
export function createPluginStreamBus(): PluginStreamBus {
  ...
  return {
    subscribe(pluginId, channel, productId, listener) {
      const key = streamKey(pluginId, channel, productId);
    ...
    publish(pluginId, channel, productId, event, eventType: StreamEventType = "message") {
      const key = streamKey(pluginId, channel, productId);
