...
export interface RunLogStore {
  begin(input: { productId: string; agentId: string; runId: string }): Promise<RunLogHandle>;
  append(
    handle: RunLogHandle,
...
function createLocalFileRunLogStore(basePath: string): RunLogStore {
  ...
  return {
    async begin(input) {
      const [productId, agentId] = safeSegments(input.productId, input.agentId);
      const runId = safeSegments(input.runId)[0]!;
      const relDir = path.join(productId, agentId);
...
export function getRunLogStore() {
  ...
  const basePath = process.env.RUN_LOG_BASE_PATH ?? path.resolve(resolveSoftclipInstanceRoot(), "data", "run-logs");
