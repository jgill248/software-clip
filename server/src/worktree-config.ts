...
import type { SoftclipConfig } from "@softclipai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Softclip worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.SOFTCLIP_IN_WORKTREE !== "true") return null;

  const configPath = resolveSoftclipConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.SOFTCLIP_WORKTREE_NAME) ??
    nonEmpty(env.SOFTCLIP_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.SOFTCLIP_INSTANCE_ID) ??
    nonEmpty(env.SOFTCLIP_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.SOFTCLIP_HOME) ??
      nonEmpty(env.SOFTCLIP_HOME) ??
      nonEmpty(env.SOFTCLIP_WORKTREES_DIR) ??
      "~/.softclip-worktrees",
  );
...
function writeConfigFile(configPath: string, config: SoftclipConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".softclip", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as SoftclipConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: SoftclipConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: SoftclipConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.SOFTCLIP_HOME = context.homeDir;
  process.env.SOFTCLIP_INSTANCE_ID = context.instanceId;
  process.env.SOFTCLIP_CONFIG = context.configPath;
  process.env.SOFTCLIP_CONTEXT = context.contextPath;
  process.env.SOFTCLIP_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SoftclipConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    SOFTCLIP_HOME: context.homeDir,
    SOFTCLIP_INSTANCE_ID: context.instanceId,
    SOFTCLIP_CONFIG: context.configPath,
    SOFTCLIP_CONTEXT: context.contextPath,
    SOFTCLIP_IN_WORKTREE: "true",
    SOFTCLIP_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: SoftclipConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SoftclipConfig;
  } catch {
    return;
  }
