 ...
 * `SOFTCLIP_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set for routes that use this allowlist.
 *
 * Examples:
 *   SOFTCLIP_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   SOFTCLIP_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.SOFTCLIP_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.SOFTCLIP_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
