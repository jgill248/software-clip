...
describe("agent instructions service", () => {
  const originalSoftclipHome = process.env.SOFTCLIP_HOME;
  const originalSoftclipInstanceId = process.env.SOFTCLIP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalSoftclipHome === undefined) delete process.env.SOFTCLIP_HOME;
    else process.env.SOFTCLIP_HOME = originalSoftclipHome;
    if (originalSoftclipInstanceId === undefined) delete process.env.SOFTCLIP_INSTANCE_ID;
    else process.env.SOFTCLIP_INSTANCE_ID = originalSoftclipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-home-");
    const externalRoot = await makeTempDir("softclip-agent-instructions-external-");
    cleanupDirs.add(softclipHome);
    cleanupDirs.add(externalRoot);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("softclip-agent-instructions-new-external-");
    cleanupDirs.add(softclipHome);
    cleanupDirs.add(externalRoot);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("softclip-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-recover-");
    cleanupDirs.add(softclipHome);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("softclip-agent-instructions-stale-root-");
    cleanupDirs.add(softclipHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("softclip-agent-instructions-heal-write-stale-");
    cleanupDirs.add(softclipHome);
    cleanupDirs.add(staleRoot);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("softclip-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(softclipHome);
    cleanupDirs.add(staleRoot);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const softclipHome = await makeTempDir("softclip-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("softclip-agent-instructions-partial-root-");
    cleanupDirs.add(softclipHome);
    cleanupDirs.add(staleRoot);
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "test-instance";
