...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Softclip OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/softclip");
    expect(text).toContain("Suggested Softclip base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("softclipApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.softclipApiUrl");
    expect(text).toContain("~/.openclaw/workspace/softclip-claimed-api-key.json");
    expect(text).toContain("SOFTCLIP_API_KEY");
