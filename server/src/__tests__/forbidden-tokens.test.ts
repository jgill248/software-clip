...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "softclip", LOGNAME: "softclip", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "softclip" }),
      },
    );

    expect(tokens).toEqual(["softclip", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "softclip" }, {
        userInfo: () => ({ username: "softclip" }),
      });

      expect(tokens).toEqual(["softclip", "custom-token"]);
    } finally {
