...
describe("agent local JWT", () => {
  const secretEnv = "SOFTCLIP_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "SOFTCLIP_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "SOFTCLIP_AGENT_JWT_ISSUER";
  const audienceEnv = "SOFTCLIP_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when SOFTCLIP_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "softclip";
    process.env[audienceEnv] = "softclip-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
