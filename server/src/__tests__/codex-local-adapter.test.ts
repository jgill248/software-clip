...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@softclipai/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@softclipai/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use softclip skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use softclip skill**" },
    ]);
  });
  ...
  it("parses command execution and file changes", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/softclipuser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
    ...
    ).toEqual([
      {
        kind: "system",
        ts,
        text: "file changes: update /Users/softclipuser/project/ui/src/pages/AgentDetail.tsx",
      },
    ]);
  });
