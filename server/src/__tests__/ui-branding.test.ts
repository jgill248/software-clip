...
<head>
    <!-- SOFTCLIP_RUNTIME_BRANDING_START -->
    <!-- SOFTCLIP_RUNTIME_BRANDING_END -->
    <!-- SOFTCLIP_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- SOFTCLIP_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from SOFTCLIP_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ SOFTCLIP_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SOFTCLIP_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SOFTCLIP_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      SOFTCLIP_IN_WORKTREE: "true",
      SOFTCLIP_WORKTREE_NAME: "softclip-pr-432",
      SOFTCLIP_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("softclip-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="softclip-worktree-name"');
    expect(meta).toContain('content="softclip-pr-432"');
    expect(meta).toContain('name="softclip-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="softclip-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="softclip-worktree-name"');
  });
});
