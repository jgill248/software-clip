...
const payload = {
  ...
  softclipWakePayloadJson: process.env.SOFTCLIP_WAKE_PAYLOAD_JSON || null,
...
type CapturePayload = {
  ...
  softclipWakePayloadJson: string | null;
...
describe("codex execute", () => {
  it("uses a Softclip-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-default-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const softclipHome = path.join(root, "softclip-home");
    ...
    const previousSoftclipHome = process.env.SOFTCLIP_HOME;
    const previousSoftclipInstanceId = process.env.SOFTCLIP_INSTANCE_ID;
    const previousSoftclipInWorktree = process.env.SOFTCLIP_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.SOFTCLIP_HOME = softclipHome;
    delete process.env.SOFTCLIP_INSTANCE_ID;
    delete process.env.SOFTCLIP_IN_WORKTREE;
    ...
    try {
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Softclip-managed Codex home"),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousSoftclipHome === undefined) delete process.env.SOFTCLIP_HOME;
      else process.env.SOFTCLIP_HOME = previousSoftclipHome;
      if (previousSoftclipInstanceId === undefined) delete process.env.SOFTCLIP_INSTANCE_ID;
      else process.env.SOFTCLIP_INSTANCE_ID = previousSoftclipInstanceId;
      if (previousSoftclipInWorktree === undefined) delete process.env.SOFTCLIP_IN_WORKTREE;
      else process.env.SOFTCLIP_IN_WORKTREE = previousSoftclipInWorktree;
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.SOFTCLIP_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects structured Softclip wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.softclipEnvKeys).toContain("SOFTCLIP_WAKE_PAYLOAD_JSON");
      expect(capture.softclipWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.softclipWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Softclip Wake Payload");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            SOFTCLIP_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the softclip heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Softclip Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the softclip heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## Softclip Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "softclip");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = "worktree-1";
    process.env.SOFTCLIP_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      expect(capture.prompt).toContain("Follow the softclip heartbeat.");
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "softclip"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "softclip"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(softclipHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousSoftclipHome === undefined) delete process.env.SOFTCLIP_HOME;
