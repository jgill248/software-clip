...
describe("shouldSilenceHttpSuccessLog", () => {
  ...
  it("silences successful polling endpoints", () => {
    ...
    expect(
      shouldSilenceHttpSuccessLog(
        "GET",
        "/api/products/5cbe79ee-acb3-4597-896e-7662742593cd/heartbeat-runs",
        200,
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      shouldSilenceHttpSuccessLog(
        "GET",
        "/api/products/5cbe79ee-acb3-4597-896e-7662742593cd/live-runs?minCount=3",
        200,
      ),
    ).toBe(true);
    expect(
      shouldSilenceHttpSuccessLog(
        "HEAD",
        "/api/products/5cbe79ee-acb3-4597-896e-7662742593cd/sidebar-badges",
        200,
      ),
    ).toBe(true);
    expect(
      shouldSilenceHttpSuccessLog(
        "GET",
        "/api/products/5cbe79ee-acb3-4597-896e-7662742593cd/issues?includeRoutineExecutions=true",
        200,
      ),
    ).toBe(true);
    expect(
      shouldSilenceHttpSuccessLog(
        "GET",
        "/api/products/5cbe79ee-acb3-4597-896e-7662742593cd/activity",
        200,
      ),
    ).toBe(true);
  });
  ...
  it("silences successful static asset requests", () => {
    expect(shouldSilenceHttpSuccessLog("GET", "/@fs/Users/dotta/softclip/ui/src/main.tsx", 200)).toBe(true);
  ...
  it("keeps failing requests visible", () => {
    ...
    expect(shouldSilenceHttpSuccessLog("GET", "/@fs/Users/dotta/softclip/ui/src/main.tsx", 404)).toBe(false);
  });
});
