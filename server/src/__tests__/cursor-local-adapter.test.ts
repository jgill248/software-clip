...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@softclipai/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@softclipai/adapter-cursor-local/ui";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$SOFTCLIP_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $SOFTCLIP_API_KEY\"";
