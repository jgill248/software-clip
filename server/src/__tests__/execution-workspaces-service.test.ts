...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Softclip Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@softclip.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("softclip-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `softclip-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "softclip-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "softclip-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "softclip-close-check",
      baseRef: "main",
      metadata: {
