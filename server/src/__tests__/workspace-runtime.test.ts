...
import { resolveSoftclipConfigPath } from "../paths.ts";
...
async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "softclip@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Softclip Test"]);
...
afterEach(async () => {
  ...
  delete process.env.SOFTCLIP_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Softclip and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/softclip",
      SOFTCLIP_HOME: "/tmp/softclip-home",
      SOFTCLIP_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.SOFTCLIP_HOME).toBeUndefined();
    expect(sanitized.SOFTCLIP_INSTANCE_ID).toBeUndefined();
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@softclipai");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/softclip-main/.git/worktrees/runtime-links\n", "utf8");
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@softclipai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/softclip-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-links-non-worktree-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-links-non-worktree-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@softclipai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".softclip", "worktrees"));
  ...
  it("rejects reusing an empty directory that only looks like a worktree because it sits inside the repo", async () => {
    ...
    const poisonedPath = path.join(repoRoot, ".softclip", "worktrees", branchName);
  ...
  it("reuses the current linked worktree instead of nesting another worktree inside it", async () => {
    ...
    const currentWorktree = path.join(repoRoot, ".softclip", "worktrees", branchName);
  ...
  it("reuses an already checked out branch from git worktree metadata even when the target path differs", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PAP-1355-worktree-reuse";
    const existingWorktree = path.join(repoRoot, ".softclip", "worktrees", branchName);
    ...
    const realized = await realizeExecutionWorkspace({
      ...
      config: {
        workspaceStrategy: {
          type: "git_worktree",
          branchTemplate: "{{issue.identifier}}-{{slug}}",
          worktreeParentDir: ".softclip/other-worktrees",
        },
      },
      issue: {
        id: "issue-1",
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$SOFTCLIP_WORKSPACE_BRANCH\" > .softclip-provision-branch",
        "printf '%s\\n' \"$SOFTCLIP_WORKSPACE_BASE_CWD\" > .softclip-provision-base",
        "printf '%s\\n' \"$SOFTCLIP_WORKSPACE_CREATED\" > .softclip-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".softclip-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".softclip-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".softclip-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".softclip-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".softclip-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".softclip-provision-version"), "utf8")).resolves.toBe("v2\n");
  });

  it("writes an isolated repo-local Softclip config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const softclipHome = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(softclipHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = instanceId;
    process.env.SOFTCLIP_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/softclip"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".softclip", "config.json");
      const envPath = path.join(workspace.cwd, ".softclip", ".env");
      ...
      expect(envVars.SOFTCLIP_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.SOFTCLIP_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.SOFTCLIP_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.SOFTCLIP_IN_WORKTREE).toBe("true");
      expect(envVars.SOFTCLIP_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveSoftclipConfigPath()).toBe(configPath);
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".softclip", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);

  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-provision-fail-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "#!/bin/sh",
          "if [ \"$1\" = \"softclip\" ] && [ \"$2\" = \"--help\" ]; then",
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"softclip\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
      ...
      try {
        await execFileAsync(scriptPath, [], {
          ...
          env: {
            ...
            SOFTCLIP_WORKSPACE_BASE_CWD: baseRoot,
            SOFTCLIP_WORKSPACE_CWD: worktreeRoot,
          },
        });
      } catch (error) {
      ...
      await expect(fs.stat(path.join(worktreeRoot, ".softclip", "config.json"))).rejects.toThrow();
      await expect(fs.stat(path.join(worktreeRoot, ".softclip", ".env"))).rejects.toThrow();
    } finally {
  ...
  it("retries worktree-local pnpm install without a frozen lockfile when the lockfile is outdated", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-outdated-lockfile-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await expect(fs.readFile(path.join(worktreeRoot, ".softclip", "config.json"), "utf8")).resolves.toContain(
        "\"database\"",
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("reattaches a missing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$SOFTCLIP_WORKSPACE_BRANCH\" > .softclip-restored-branch",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".softclip-restored-branch"), "utf8")).resolves.toBe(`${branchName}\n`);
  ...
  it("reprovisions an existing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.rm(path.join(initial.cwd, ".softclip-restored-state"), { force: true });
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".softclip-restored-state"), "utf8")).resolves.toBe("reprovisioned\n");
  });
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".softclip", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.SOFTCLIP_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              SOFTCLIP_HOME: "{{workspace.cwd}}/.softclip/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".softclip", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".softclip", "runtime-services"));
  });

  it("does not leak parent Softclip instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "softclipConfig: process.env.SOFTCLIP_CONFIG ?? null,",
          "softclipHome: process.env.SOFTCLIP_HOME ?? null,",
          "softclipInstanceId: process.env.SOFTCLIP_INSTANCE_ID ?? null,",
    ...
    process.env.SOFTCLIP_CONFIG = "/tmp/base-softclip-config.json";
    process.env.SOFTCLIP_HOME = "/tmp/base-softclip-home";
    process.env.SOFTCLIP_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/softclip";
    ...
    expect(captured.softclipConfig).toBeNull();
    expect(captured.softclipHome).toBeNull();
    expect(captured.softclipInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-sibling-"));
  ...
  it("starts only the selected workspace-controlled runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-control-start-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("stops only the selected execution workspace runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-control-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("softclip-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-reconcile-"));
    const softclipHome = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-home-"));
    process.env.SOFTCLIP_HOME = softclipHome;
    process.env.SOFTCLIP_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("marks persisted local services stopped when the registry pid is stale", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "softclip-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:softclip-dev`,
      command: "pnpm dev",
      cwd: "/tmp/softclip-primary",
      port: 49195,
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-softclip-dev-stale",
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-runtime-stop-persisted-"));
    const productId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
