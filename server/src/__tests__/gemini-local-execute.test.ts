...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects softclip env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the softclip heartbeat.");
      expect(promptArg).toContain("Softclip runtime note:");
      ...
      expect(invocationPrompt).toContain("Softclip API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "softclip-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Softclip Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the softclip heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
