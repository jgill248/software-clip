...
import { createAssetImageMetadataSchema } from "@softclipai/shared";
...
export function assetRoutes(db: Db, storage: StorageService) {
  ...
  router.post("/products/:productId/assets/images", async (req, res) => {
    ...
    const asset = await svc.create(productId, {
  ...
  router.get("/assets/:assetId/content", async (req, res, next) => {
    ...
    assertCompanyAccess(req, asset.productId);

    const object = await storage.getObject(asset.productId, asset.objectKey);
