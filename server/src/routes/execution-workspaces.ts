...
export function executionWorkspaceRoutes(db: Db) {
  ...
  async function handleExecutionWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Softclip can run workspace commands" });
      return;
    }
      ...
      run: async () => {
        ...
        if (action === "run") {
          ...
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Softclip can run workspace commands");
          }
        ...
        if (action === "start" || action === "restart") {
          const availableWorkspace = await ensureWorkspaceAvailable();
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Softclip can manage local runtime services");
          }
