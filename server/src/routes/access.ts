...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "softclip" &&
    normalized !== "softclip-create-agent" &&
    normalized !== "softclip-create-plugin" &&
...
/** Resolve the Softclip repo skills directory (built-in / managed skills). */
function resolveSoftclipSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const softclipSkillsDir = resolveSoftclipSkillsDir();

  // Build set of Softclip-managed skill names
  const softclipSkillNames = new Set<string>();
  if (softclipSkillsDir) {
    try {
      for (const entry of fs.readdirSync(softclipSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) softclipSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isSoftclipManaged: softclipSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.softclipApiUrl)) {
    const legacySoftclipApiUrl = nonEmptyTrimmedString(input.softclipApiUrl);
    if (legacySoftclipApiUrl) merged.softclipApiUrl = legacySoftclipApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    softclipApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.softclipApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawSoftclipApiUrl =
    typeof defaults.softclipApiUrl === "string"
      ? defaults.softclipApiUrl.trim()
      : "";
  if (rawSoftclipApiUrl) {
    try {
      const parsedSoftclipApiUrl = new URL(rawSoftclipApiUrl);
      if (
        parsedSoftclipApiUrl.protocol !== "http:" &&
        parsedSoftclipApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_softclip_api_url_protocol",
          level: "warn",
          message: `softclipApiUrl must use http:// or https:// (got ${parsedSoftclipApiUrl.protocol}).`
        });
      } else {
        normalized.softclipApiUrl = parsedSoftclipApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_softclip_api_url_configured",
          level: "info",
          message: `softclipApiUrl set to ${parsedSoftclipApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_softclip_api_url_invalid",
        level: "warn",
        message: `Invalid softclipApiUrl: ${rawSoftclipApiUrl}`
      });
    }
  }
...
function actorHasActiveUserMembership(req: Request, productId: string) {
  return (
    ...
    req.actor.memberships.some(
      (membership) =>
        membership.productId === productId && membership.status === "active",
    )
  );
}
...
async function loadCompanyAccessSummary(
  ...
  const membership =
    userId ? await access.getMembership(productId, "user", userId) : null;
  const [canManageMembers, canInviteUsers, canApproveJoinRequests] =
    await Promise.all([
      access.canUser(productId, userId, "users:manage_permissions"),
      access.canUser(productId, userId, "users:invite"),
      access.canUser(productId, userId, "joins:approve"),
    ]);
...
async function loadCompanyMemberRecords(db: Db, productId: string) {
...
async function loadCompanyUserDirectory(db: Db, productId: string) {
  const members = await db
    .select({
...
async function loadCompanyInviteRecords(
  ...
  const rows = await db
    .select()
    .from(invites)
    .where(whereClause ? and(eq(invites.productId, productId), whereClause) : eq(invites.productId, productId))
  ...
  const [userMap, joinRows, companyName] = await Promise.all([
    ...
    visibleRows.length
      ? db
          ...
          .where(
            and(
              eq(joinRequests.productId, productId),
              inArray(
...
async function loadJoinRequestRecords(db: Db, productId: string) {
  const rows = collapseDuplicatePendingHumanJoinRequests(
    await db
      .select()
      .from(joinRequests)
      .where(eq(joinRequests.productId, productId))
...
async function loadUserCompanyAccessResponse(
  ...
  const productIds = [...new Set(memberships.map((membership) => membership.productId))];
  const companyRows = productIds.length
    ? await db
        .select({
          id: products.id,
          name: products.name,
  ...
  return {
    ...
    companyAccess: memberships.map((membership) => {
      const company = companyMap.get(membership.productId) ?? null;
      return {
        ...membership,
        principalType: "user" as const,
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Softclip host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Softclip is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm softclip allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/softclip`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/softclip-claimed-api-key.json and load SOFTCLIP_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: softclipApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Softclip hostname is reachable and allowed via `pnpm softclip allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Softclip API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/softclip/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Softclip OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          softclipApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Softclip generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "softclipApiUrl": "https://softclip-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Softclip before key claim is allowed.
    ...
    - ~/.openclaw/workspace/softclip-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/softclip-claimed-api-key.json

    And set the SOFTCLIP_API_KEY and SOFTCLIP_API_URL in your environment variables as specified here:
    ...
    Then set SOFTCLIP_API_KEY and SOFTCLIP_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Softclip skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your SOFTCLIP_API_URL to the top of your skill and note the path to your SOFTCLIP_API_URL
    ...
    ${
      ...
      "Ensure Softclip is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Softclip base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.softclipApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm softclip allowed-hostname <host>
      - then restart Softclip and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@softclip.local";
...
async function resolveAcceptedInviteJoinRequest(
  ...
  if (!invite.productId) return null;
  ...
  return findReusableHumanJoinRequest(
    await db
      ...
      .where(
        and(
          eq(joinRequests.productId, invite.productId),
...
export function accessRoutes(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (approved.status === "approved") {
        const productIds = await boardAuth.resolveBoardActivityCompanyIds({
          userId,
  ...
  router.get("/cli-auth/me", async (req, res) => {
    ...
    res.json({
      ...
      productIds: accessSnapshot.productIds,
  ...
  async function assertCanGenerateOpenClawInvitePrompt(
    ...
    if (req.actor.type !== "board") throw unauthorized();
    if (isLocalImplicit(req)) return;
    const allowed = await access.canUser(productId, req.actor.userId, "users:invite");
    if (!allowed) throw forbidden("Permission denied");
  }
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "softclip", path: "/api/skills/softclip" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "softclip-create-agent",
          path: "/api/skills/softclip-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const companyBranding = await getInviteCompanyBranding(created.productId, token);
  ...
  router.post(
    ...
    async (req, res) => {
      const productId = req.params.productId as string;
      await assertCanGenerateOpenClawInvitePrompt(req, productId);
      const { token, created, normalizedAgentMessage } =
        await createCompanyInviteForCompany({
          req,
          productId,
  ...
  router.get("/invites/:token", async (req, res) => {
    ...
    const companyBranding = await getInviteCompanyBranding(invite.productId, token);
  ...
  router.get("/invites/:token/onboarding", async (req, res) => {
    ...
    const companyBranding = await getInviteCompanyBranding(invite.productId);
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "softclip",
          path: `/api/invites/${token}/skills/softclip`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "softclip") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const productId = invite.productId;
      if (!productId) throw conflict("Invite is missing company scope");
      if (
      ...
      if (
        requestType === "human" &&
        actorHasActiveUserMembership(req, productId)
      ) {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              softclipApiUrl: req.body.softclipApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.softclipApiUrl &&
          !persistedDefaults.softclipApiUrl
        ) {
          missingPersistedFields.push("softclipApiUrl");
        }
  ...
  router.post("/invites/:inviteId/revoke", async (req, res) => {
    ...
    if (invite.inviteType === "bootstrap_ceo") {
      ...
      if (!invite.productId) throw conflict("Invite is missing company scope");
      await assertCompanyPermission(req, invite.productId, "users:invite");
    }
  ...
  router.get("/products/:productId/invites", async (req, res) => {
    ...
    const invitesForCompany = await loadCompanyInviteRecords(db, productId, query);
  ...
  router.get("/products/:productId/join-requests", async (req, res) => {
    ...
    const all = await loadJoinRequestRecords(db, productId);
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (existing.requestType === "human") {
        ...
        const existingAgents = await agents.list(productId);
        ...
        const created = await agents.create(productId, {
          name: agentName,
          role: "general",
          title: null,
          status: "idle",
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      await logActivity(db, {
        productId: joinRequest.productId,
        actorType: "system",
        actorId: "join-claim",
  ...
  router.get("/products/:productId/members", async (req, res) => {
    ...
    const [members, currentAccess] = await Promise.all([
      loadCompanyMemberRecords(db, productId),
      loadCompanyAccessSummary(req, access, productId),
    ]);
    res.json({
      members,
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      const member = (await loadCompanyMemberRecords(db, productId)).find(
