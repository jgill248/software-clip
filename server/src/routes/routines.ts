...
import { trackRoutineCreated } from "@softclipai/shared/telemetry";
...
export function routineRoutes(db: Db) {
  ...
  async function assertBoardCanAssignTasks(req: Request, productId: string) {
  ...
  function assertCanManageCompanyRoutine(req: Request, productId: string, assigneeAgentId?: string | null) {
    assertCompanyAccess(req, productId);
    if (req.actor.type === "board") return;
  ...
  async function assertCanManageExistingRoutine(req: Request, routineId: string) {
    ...
    assertCompanyAccess(req, routine.productId);
  ...
  router.post("/products/:productId/routines", validate(createRoutineSchema), async (req, res) => {
    ...
    const created = await svc.create(productId, req.body, {
  ...
  router.get("/routines/:id", async (req, res) => {
    ...
    assertCompanyAccess(req, detail.productId);
    res.json(detail);
  });
  ...
  router.patch("/routines/:id", validate(updateRoutineSchema), async (req, res) => {
    ...
    if (assigneeWillChange) {
      await assertBoardCanAssignTasks(req, routine.productId);
    }
  ...
  router.post("/routine-triggers/public/:publicId/fire", async (req, res) => {
    const result = await svc.firePublicTrigger(req.params.publicId as string, {
      ...
      signatureHeader: req.header("x-softclip-signature"),
      hubSignatureHeader: req.header("x-hub-signature-256"),
      timestampHeader: req.header("x-softclip-timestamp"),
