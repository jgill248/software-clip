...
import { trackProjectCreated } from "@softclipai/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Softclip can run workspace commands" });
      return;
    }
