...
function extractUiParserSource(
  ...
  const contractVersion = pkg.softclip?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no softclip.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
