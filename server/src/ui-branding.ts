const FAVICON_BLOCK_START = "<!-- SOFTCLIP_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- SOFTCLIP_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- SOFTCLIP_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- SOFTCLIP_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.SOFTCLIP_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.SOFTCLIP_WORKTREE_NAME) ?? nonEmpty(env.SOFTCLIP_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.SOFTCLIP_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="softclip-worktree-enabled" content="true" />',
    `<meta name="softclip-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="softclip-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="softclip-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
