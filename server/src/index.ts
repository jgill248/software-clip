...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.SOFTCLIP_SECRETS_PROVIDER === undefined) {
    process.env.SOFTCLIP_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.SOFTCLIP_SECRETS_STRICT_MODE === undefined) {
    process.env.SOFTCLIP_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.SOFTCLIP_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.SOFTCLIP_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.SOFTCLIP_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.SOFTCLIP_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set SOFTCLIP_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@softclip.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.SOFTCLIP_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://softclip:softclip@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "softclip");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://softclip:softclip@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "softclip");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: softclip");
    }
  
    const embeddedConnectionString = `postgres://softclip:softclip@127.0.0.1:${port}/softclip`;
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.SOFTCLIP_LISTEN_HOST = runtimeListenHost;
  process.env.SOFTCLIP_LISTEN_PORT = String(listenPort);
  if (!process.env.SOFTCLIP_API_URL) {
    process.env.SOFTCLIP_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  }
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.SOFTCLIP_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.SOFTCLIP_API_URL!,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Softclip server failed to start");
    process.exit(1);
  });
}
