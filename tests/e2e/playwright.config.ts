...
const PORT = Number(process.env.SOFTCLIP_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const SOFTCLIP_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "softclip-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp SOFTCLIP_HOME.
  webServer: {
    command: `pnpm softclip onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Softclip home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      SOFTCLIP_HOME,
      SOFTCLIP_INSTANCE_ID: "playwright-e2e",
      SOFTCLIP_BIND: "loopback",
      SOFTCLIP_DEPLOYMENT_MODE: "local_trusted",
