...
export function useProjectOrder({ projects, productId, userId }: UseProjectOrderParams) {
  const queryClient = useQueryClient();
  const queryKey = useMemo(
    () => queryKeys.sidebarPreferences.projectOrder(productId ?? "__none__", userId ?? "__anon__"),
    [productId, userId],
  );

  const { data } = useQuery({
    queryKey,
    queryFn: () => sidebarPreferencesApi.getProjectOrder(productId!),
  ...
  const mutation = useMutation({
    mutationFn: (nextIds: string[]) => sidebarPreferencesApi.updateProjectOrder(productId!, { orderedIds: nextIds }),
  ...
  const persistOrder = useCallback(
    ...
    [productId, mutation, projects, queryClient, queryKey, userId],
  );
