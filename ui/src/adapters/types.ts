...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@softclipai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@softclipai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@softclipai/adapter-utils").TranscriptEntry[];
