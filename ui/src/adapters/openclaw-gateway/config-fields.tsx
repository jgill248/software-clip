...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Softclip API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "softclipApiUrl",
                  String(config.softclipApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "softclipApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://softclip.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/softclip-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "softclip"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Softclip persists a device key during onboarding so pairing approvals
