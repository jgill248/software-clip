...
function ProjectIssuesList({ projectId, productId }: { projectId: string; productId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="softclip:project-issues-view"
...
export function ProjectDetail() {
  ...
  // Softclip pivot §6: per-project budget panel removed. Dev teams don't
  ...
  // Softclip pivot §6: projectBudgetSummary memo and budgetMutation removed.
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`softclip:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`softclip:project-tab:${project.id}`, tab); } catch {}
    }
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 space-y-2">
          ...
          />
          {/* Softclip pivot §6: budget-hard-stop pause chip removed. */}
        </div>
      </div>
      ...
      {/* Softclip pivot §6: Budget tab content removed. */}
