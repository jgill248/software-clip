...
describe("InviteLandingPage", () => {
  ...
  it("defaults invite auth to account creation and guides existing users back to sign in", async () => {
    ...
    expect(container.textContent).toContain("You've been invited to join Softclip");
    ...
    expect(localStorage.getItem("softclip:pending-invite-token")).toBe("pcp_invite_test");
    expect(container.querySelector('div[aria-label="Acme Robotics"]')).not.toBeNull();
  ...
  it("auto-accepts the invite after account creation and redirects into the company", async () => {
    ...
    expect(localStorage.getItem("softclip:pending-invite-token")).toBeNull();
