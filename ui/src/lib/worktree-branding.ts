...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("softclip-worktree-enabled") !== "true") return null;

  const name = readMetaContent("softclip-worktree-name");
  const color = normalizeHexColor(readMetaContent("softclip-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("softclip-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
