...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`softclip-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="softclip-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("softclip-mermaid-status", error && "softclip-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="softclip-mermaid-source">
...
export function MarkdownBody({
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `softclip-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "softclip-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "softclip-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
