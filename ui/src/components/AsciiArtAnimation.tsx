...
type SoftclipSprite = (typeof SOFTCLIP_SPRITES)[number];
...
function spriteSize(sprite: SoftclipSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SOFTCLIP_SPRITES[(baseCol + baseRow) % SOFTCLIP_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SOFTCLIP_SPRITES[Math.floor(Math.random() * SOFTCLIP_SPRITES.length)]!;
