...
This document explains how Softclip interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

Softclip separates four concepts that are easy to blur together:
...
This is a hard invariant. Softclip is single-assignee by design.

## 3. Status Semantics

Softclip issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, Softclip may still need a wake path to ensure the assignee actually sees it
...
- Softclip can wake the assignee
- Softclip can track runs linked to the issue
- Softclip can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- Softclip can track the ownership and status
- Softclip cannot rely on heartbeat/run semantics to keep them moving
...
Softclip already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

Softclip uses two different relationships for different jobs.
...
For agent-assigned, non-terminal, actionable issues, Softclip should not leave work in a state where nobody is working it and nothing will wake it.
...
Softclip now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, Softclip queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, Softclip moves the issue to `blocked` and posts a visible comment
...
- Softclip queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, Softclip moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, Softclip now does three things in sequence:
...
That is the execution contract Softclip should present to operators.
