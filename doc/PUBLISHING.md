...
Low-level reference for how Softclip packages are prepared and published to npm.
...
Softclip no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `softclip`, imports code from workspace packages such as:
...
### Manual first publish for `@softclipai/ui`
...
npm view @softclipai/ui version

# make sure the dist payload is fresh
pnpm --filter @softclipai/ui build
...
- If `npm view @softclipai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@softclipai/ui`, the `softclip` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Softclip uses calendar versions:
