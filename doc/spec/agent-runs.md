...
This spec defines how Softclip actually runs agents while staying runtime-agnostic.
...
1. Softclip is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Softclip manages lifecycle, and on exit Softclip parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `softclip.skill` (shared Softclip skill text block)
...
2. Preferred transport is env vars (`SOFTCLIP_*`) injected at runtime.
