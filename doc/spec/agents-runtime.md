...
For `claude_local` and `codex_local`, Softclip assumes the CLI is already installed and authenticated on the host machine.
...
Softclip stores resumable session state per `(agent, taskKey, adapterType)`.
