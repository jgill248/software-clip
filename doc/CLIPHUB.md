...
ClipHub is the public registry where people share, discover, and download Softclip company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Softclip what a package registry is to a programming language. Softclip already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Softclip — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Softclip instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Softclip spec. It contains:
...
**Install (fresh start):**
```
softclip install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Softclip instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Softclip, export your company as a template, then publish:

```
softclip export --template my-company
softclip publish cliphub my-company
```
...
ClipHub is a **separate service** from Softclip itself. Softclip is self-hosted; ClipHub is a hosted registry that Softclip instances talk to.
...
| **Softclip CLI** | `softclip install`, `softclip publish`, `softclip cliphub sync` — built into Softclip |
| **Softclip UI** | "Browse ClipHub" panel in the Softclip web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Softclip) |
| Backend | TypeScript + Hono (consistent with Softclip) |
| Database | PostgreSQL |
...
4. Run `softclip install cliphub:acme/lean-saas-shop`
5. Softclip creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Softclip until it works well
2. Export: `softclip export --template my-agency`
3. Publish: `softclip publish cliphub my-agency`
...
3. Install just that agent: `softclip install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Softclip

ClipHub is **not required** to use Softclip. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Softclip what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `softclip install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Softclip web UI
- [ ] `softclip cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Softclip's own philosophy)
