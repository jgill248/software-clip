...
Softclip V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.softclip/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.softclip/instances/default/data/storage` (`local_disk`)
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Softclip runtime-owned keys are injected
...
- `.softclip.yaml` sidecar for Softclip-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.softclip.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Softclip routine fidelity in `.softclip.yaml`
- Softclip imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.softclip.yaml`
