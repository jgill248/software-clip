...
Softclip supports two runtime modes:
...
Softclip now treats **bind** as a separate concern from auth:
...
pnpm softclip onboard --yes --bind lan
pnpm softclip run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Softclip emits a startup warning with a one-time high-entropy claim URL.
