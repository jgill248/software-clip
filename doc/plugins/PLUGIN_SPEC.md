# Softclip Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Softclip's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Softclip app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Softclip HTTP APIs directly.
...
Softclip plugin design is based on the following assumptions:

1. Softclip is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Softclip business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Softclip core.
...
2. Let plugins add major capabilities without editing Softclip core.
...
4. Depend on project-local plugin folders such as `.softclip/plugins`.
...
The single Softclip deployment an operator installs and controls.

### 5.2 Company

A first-class Softclip business object inside the instance.
...
A trusted in-process extension loaded directly by Softclip core.
...
An installable instance-wide extension package loaded through the Softclip plugin runtime.
...
Softclip has two extension classes.
...
Softclip already has a concrete workspace model:
...
Plugins live under the Softclip instance directory.

Suggested layout:

- `~/.softclip/instances/default/plugins/package.json`
- `~/.softclip/instances/default/plugins/node_modules/`
- `~/.softclip/instances/default/plugins/.cache/`
- `~/.softclip/instances/default/data/plugins/<plugin-id>/`
...
Softclip should add CLI commands:

- `pnpm softclip plugin list`
- `pnpm softclip plugin install <package[@version]>`
- `pnpm softclip plugin uninstall <plugin-id>`
- `pnpm softclip plugin upgrade <plugin-id> [version]`
- `pnpm softclip plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@softclip/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface SoftclipPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Softclip agents can use during runs.
...
- Softclip server starts one worker process per installed plugin
...
Receives one typed Softclip domain event.
...
export function definePlugin(definition: PluginDefinition): SoftclipPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: SoftclipPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@softclipai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@softclipai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@softclipai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@softclipai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@softclipai/plugin-sdk/ui`
...
The `@softclipai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Softclip secret provider system rather than a plain text input
...
1. Core Softclip data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Softclip objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Softclip product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Softclip later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Softclip secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm softclip plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Softclip server. This is a normative requirement, not optional.
...
### 27.1 `@softclipai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@softclipai/plugin-test-harness";
...
For developing a plugin against a running Softclip instance:

- The operator installs the plugin from a local path: `pnpm softclip plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-softclip-plugin`) that scaffolds:

- `package.json` with correct `softclipPlugin` keys
...
- `@softclip/plugin-workspace-files`
- `@softclip/plugin-terminal`
- `@softclip/plugin-git`
- `@softclip/plugin-linear`
- `@softclip/plugin-github-issues`
- `@softclip/plugin-grafana`
- `@softclip/plugin-runtime-processes`
- `@softclip/plugin-stripe`
...
- `@softclipai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@softclipai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@softclipai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@softclipai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@softclipai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@softclipai/plugin-sdk/ui`
...
- `@softclipai/plugin-test-harness`
- `create-softclip-plugin` starter template
...
Softclip should not implement a generic in-process hook bag modeled directly after local coding tools.

Softclip should implement:
...
That is the complete target design for the Softclip plugin system.
