...
This guide describes the current, implemented way to create a Softclip plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Softclip app.
...
```bash
pnpm --filter @softclipai/create-softclip-plugin build
node packages/plugins/create-softclip-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Softclip repo:

```bash
pnpm --filter @softclipai/create-softclip-plugin build
node packages/plugins/create-softclip-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@softclipai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@softclipai/plugin-sdk` from the local Softclip checkout into a `.softclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Softclip from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@softclipai/plugin-sdk/ui`
